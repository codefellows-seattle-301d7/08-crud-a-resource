(function(module) {
  ...
  Article.createTable = function() {
    webDB.execute(
      'CREATE TABLE IF NOT EXISTS arts (id INTEGER PRIMARY KEY, title VARCHAR, category VARCHAR, author VARCHAR, authorURL VARCHAR, publishedOn VARCHAR, body VARCHAR);', // TODO: What SQL command do we run here inside these quotes?
      function() {
  ...
  Article.fetchAll = function(nextFunction) {
    ...
    webDB.execute('SELECT * FROM arts', function(rows) { // TODO: fill these quotes to 'select' our table.
      if (rows.length) {
        /* TODO:
      ...
      [
        {
          ...
          /* Note: this is an advanced admin option, so you will need to test
              out an individual query in the console */
          'sql': 'DELETE FROM arts WHERE id = ?;', // <--- complete the command here, inside the quotes;
          'data': [this.id]
        }
      ]
    );
  ...
  Article.truncateTable = function() {
    webDB.execute(
      ...
      'DELETE * FROM arts;' // <----finish the command here, inside the quotes.
    );
  };
